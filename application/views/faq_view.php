...
<html lang="en">
    <head>
        ...
        <script type="text/javascript">
            error = '<?php echo $error; ?>';
                    ...
                    site_url = '<?php echo site_url(); ?>/faq/';
            ...
            function form_edit(index)
            {
 				if(_edit!='0'){
                    ...
                    $.ajax({
                    type: "POST",
                            url: "<?php echo site_url('faq/faq_show_by_id') ?>",
                            timeout: 20000,
                            data:
                            ...
                            var arr = JSON.parse(result);
                                    $('#faq_ask').val(arr[0].faq_ask);
                                    $('#faq_answer').val(arr[0].faq_answer);
            ...
            $.ajax({
            type: "POST",
                    url: "<?php echo site_url('faq/delete_permanent') ?>",
                    timeout: 20000,
                    data:
                    'datamodel=' + $(index).attr("datamodel")
                    ...
                    alertify.success('Hapus sukses');
                            location.reload();
                            window.location.replace("<?php echo site_url('faq/'); ?>");
                    }
                    else if (result == "2")
                    {
            ...
            var app = angular.module('main', ['ngTable', 'ngTableExport']).
                    ...
                    var data = [];
                            $http.get('<?php echo site_url('faq/faq_show'); ?>')
                            ...
                            data = $user;
                                    ...
                                    page: 1, // show first page
                                            ...
                                            filter: {
                                            faq_ask: '', // initial filter
                                            },
                                            sorting: {
                                            faq_ask: 'asc'    // initial sorting
                                            }
    ...
    <body ng-app="main" style="overflow-x:hidden;background-image:url('<?php echo base_url(); ?>include_front/img/felice_trans.png'),linear-gradient(#ffe4eb, #ff799e);background-repeat: no-repeat;background-position: center;">
                    ...
                    <div ng-controller="DemoCtrl">
						...
						<div class="row" >
							...
							<div class="col-md-2" style="padding-right:30px" >
                                <input class="form-control" type="search" id="filter_search" ng-model="faq.user_name" placeholder="Cari Data..." aria-label="Cari Data" />
                            </div>
                        </div>
                        <div class="row" >
                            <div class="col-md-12">
                                <table ng-table="tableParams"  class="table table-striped ng-table-responsive" export-csv="csv">
                                    <tr ng-repeat="(datamodel, faq) in $data | filter:faq.user_name as results" ng-class="{ 'emphasis': faq.role_nama == '<?php echo $this->session->userdata("role_nama") ?>'}">
                                        <td class="faq" data-title="'Pertanyaan'" sortable="'faq'">
                                            <span id="user_name_{{$index}}" >{{faq.faq_ask}}</span>
                                        </td> 
                                        <td class="faq" data-title="'Jawaban'" sortable="'faq'">
                                            <span id="user_name_{{$index}}" >{{faq.faq_answer}}</span>
                                        </td> 


                                        <td class="action" data-title="'Actions'" >
                                    <center><a href="" id="faq-{{$index}}" datamodel={{faq.datamodel}} class="btn btn-default btn-xs" onclick="form_edit(this)"><img src="<?php echo base_url(); ?>include/img/edit.png" /></a>   
                                        <a href="" id="faq-{{$index}}" datamodel={{faq.datamodel}} class="btn btn-default btn-xs" onclick="deleted(this)"><img src="<?php echo base_url(); ?>include/img/delete.png" /></a>   </center>
                                    </td>

                                    <a style="display:none" >
                                        <span >{{faq.datamodel}}</span>
                                    </a>
                                    </tr> 
        ...
        <?php $this->load->view("faq_crud_view"); // remodal  ?>
