                        ...
                        <div class="row" >
                            <div class="col-md-12">
                                <table ng-table="tableParams"  class="table table-striped ng-table-responsive" export-csv="csv">
                                    <tr ng-repeat="(datamodel, role) in $data | filter:role.role_nama as results" ng-class="{ 'emphasis': role.role_nama == '<?php echo $this->session->userdata("role_nama") ?>'}">
                                        <td class="col-md-4 borderkanan" data-title="'Role'" sortable="'role_nama'">
                                            <span id="role_nama_{{$index}}" >{{role.role_nama}}</span>
                                        </td> 
                                        <td class="col-md-7" data-title="'Keterangan'" sortable="'role_keterangan'"> 
                                            <span id="role_keterangan{{$index}}" >{{role.role_keterangan}}</span>  
                                        </td>

                                        <td class="action" data-title="'Actions'" >
                                    <center><a href="" id="role-{{$index}}" datamodel={{role.datamodel}} class="btn btn-default btn-xs" onclick="form_edit(this)"><img src="<?php echo base_url(); ?>include/img/edit.png" /></a>   
                                        <a href="" id="role-{{$index}}" datamodel={{role.datamodel}} class="btn btn-default btn-xs" onclick="deleted(this)"><img src="<?php echo base_url(); ?>include/img/delete.png" /></a>   </center>
                                    </td>
