...
<html lang="en">
    <head>
        ...
        <script type="text/javascript">
            error = '<?php echo $error; ?>';
                    ...
                    site_url = '<?php echo site_url(); ?>/slider/';
            ...
            function form_edit(index)
            {
 				if(_edit!='0'){
                    ...
                    $.ajax({
                    type: "POST",
                            url: "<?php echo site_url('slider/slider_show_by_id') ?>",
                            timeout: 20000,
                            data:
            ...
            $.ajax({
            type: "POST",
                    url: "<?php echo site_url('slider/delete_permanent') ?>",
                    timeout: 20000,
                    data:
                    'datamodel=' + $(index).attr("datamodel")
                    ...
                    alertify.success('Hapus sukses');
                            location.reload();
                            window.location.replace("<?php echo site_url('slider/'); ?>");
                    }
                    else if (result == "2")
                    {
            ...
            var app = angular.module('main', ['ngTable', 'ngTableExport']).
                    ...
                    var data = [];
                            $http.get('<?php echo site_url('slider/slider_show'); ?>')
    ...
    <body ng-app="main" style="overflow-x:hidden;background-image:url('<?php echo base_url(); ?>include_front/img/felice_trans.png'),linear-gradient(#ffe4eb, #ff799e);background-repeat: no-repeat;background-position: center;">
                    ...
                    <div ng-controller="DemoCtrl">
						...
						<div class="row" >
							...
							<div class="col-md-2" style="padding-right:30px" >
                                <input class="form-control" type="search" id="filter_search" ng-model="slider.user_name" placeholder="Cari Data..." aria-label="Cari Data" />
                            </div>
                        </div>
                        <div class="row" >
                            <div class="col-md-12">
                                <table ng-table="tableParams"  class="table table-striped ng-table-responsive" export-csv="csv">
                                    <tr ng-repeat="(datamodel, slider) in $data | filter:slider.user_name as results" ng-class="{ 'emphasis': slider.role_nama == '<?php echo $this->session->userdata("role_nama") ?>'}">
										<td align="center" class="col-md-2 borderkanan" data-title="'Foto'" sortable="'slider_foto'">
											<img width="50%" src="<?php echo base_url(); ?>/include_front/img/slider/{{slider.foto}}" />
                                        </td> 
                                        <td class="action" data-title="'Actions'" >
                                    <center><a href="" id="slider-{{$index}}" datamodel={{slider.datamodel}} class="btn btn-default btn-xs" onclick="form_edit(this)"><img src="<?php echo base_url(); ?>include/img/edit.png" /></a>     </center>
                                    </td>

                                    <a style="display:none" >
                                        <span >{{slider.datamodel}}</span>
                                    </a>
                                    </tr> 
        ...
        <?php $this->load->view("slider_crud_view"); // remodal  ?>
