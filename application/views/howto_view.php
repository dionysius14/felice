...
<html lang="en">
    <head>
        ...
        <script type="text/javascript">
            error = '<?php echo $error; ?>';
                    ...
                    site_url = '<?php echo site_url(); ?>/howto/';
            ...
            function form_edit(index)
            {
 				if(_edit!='0'){
                    ...
                    $.ajax({
                    type: "POST",
                            url: "<?php echo site_url('howto/howto_show_by_id') ?>",
                            timeout: 20000,
                            data:
            ...
            $.ajax({
            type: "POST",
                    url: "<?php echo site_url('howto/delete_permanent') ?>",
                    timeout: 20000,
                    data:
                    'datamodel=' + $(index).attr("datamodel")
                    ...
                    alertify.success('Hapus sukses');
                            location.reload();
                            window.location.replace("<?php echo site_url('howto/'); ?>");
                    }
                    else if (result == "2")
                    {
            ...
            var app = angular.module('main', ['ngTable', 'ngTableExport']).
                    ...
                    var data = [];
                            $http.get('<?php echo site_url('howto/howto_show'); ?>')
                            ...
                            data = $user;
                                    ...
                                    page: 1, // show first page
                                            ...
                                            filter: {
                                            howto_step: '', // initial filter
                                            },
                                            sorting: {
                                            howto_step: 'asc'    // initial sorting
                                            }
    ...
    <body ng-app="main" style="overflow-x:hidden;background-image:url('<?php echo base_url(); ?>include_front/img/felice_trans.png'),linear-gradient(#ffe4eb, #ff799e);background-repeat: no-repeat;background-position: center;">
                    ...
                    <div ng-controller="DemoCtrl">
						...
						<div class="row" >
							...
							<div class="col-md-2" style="padding-right:30px" >
                                <input class="form-control" type="search" id="filter_search" ng-model="howto.user_name" placeholder="Cari Data..." aria-label="Cari Data" />
                            </div>
                        </div>
                        <div class="row" >
                            <div class="col-md-12">
                                <table ng-table="tableParams"  class="table table-striped ng-table-responsive" export-csv="csv">
                                    ...
                                    <center><a href="" id="howto-{{$index}}" datamodel={{howto.datamodel}} class="btn btn-default btn-xs" onclick="form_edit(this)"><img src="<?php echo base_url(); ?>include/img/edit.png" /></a>   
                                        <a href="" id="howto-{{$index}}" datamodel={{howto.datamodel}} class="btn btn-default btn-xs" onclick="deleted(this)"><img src="<?php echo base_url(); ?>include/img/delete.png" /></a>   </center>
                                    </td>

                                    <a style="display:none" >
                                        <span >{{howto.datamodel}}</span>
                                    </a>
                                    </tr> 
        ...
        <?php $this->load->view("howto_crud_view"); // remodal  ?>
